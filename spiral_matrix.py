...
def find_center(output):
    ...
    # output_center += [a]
    print("output_center", len(output))
...
def spiral_matrix(side: int, string: str) -> list:
    ...
    # Update input string to final length of values
    string = enrich_input_string(string, output)
    print("enriched_string", string)
    ...
    count = len(string)
    # Then go right
    output_center[a + 1][a + 1] = list_string.pop(0)
    # Then go down
    # Then go left twice
    # Then up twice
    # Then right twice - Three if can - += here
